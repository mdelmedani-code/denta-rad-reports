-- Update get_unbilled_reports to include patient_internal_id
...
BEGIN
  ...
  WHERE cases.billed = false
    ...
    AND (p_start_date IS NULL OR r.completed_at::date >= p_start_date)
    AND (p_end_date IS NULL OR r.completed_at::date <= p_end_date)
  GROUP BY c.name, c.contact_email
  ORDER BY c.name;
END;
$function$;

-- Create default invoice template settings
INSERT INTO pdf_template_settings (setting_key, setting_value)
VALUES (
  'invoice_template',
  jsonb_build_object(
    ...
    'table_columns', jsonb_build_array('description', 'date', 'field_of_view', 'amount')
  )
)
